...
class QuizMenu(object):
    ...
    def newQuizQuestion(self):
        ...
        for elem in self.data.playerEntries:
            self.canvas.delete(elem)
            
        for i in range(len(getQuizSolution(self.quizPath))):
            e = Entry(self.canvas)
            self.data.playerEntries.append(self.canvas.create_window(width//2,
    ...
    def draw(self, data, canvas):
        height = data.height
        width = data.width
        #canvas.delete(data.currentDisplay)
        #border
